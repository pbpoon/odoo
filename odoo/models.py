...
class BaseModel(MetaModel('DummyModel', (object,), {'_register': False})):
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def write(self, vals):
        ...
        * .. _openerp/models/relationals/format:
        ...
        with self.env.protecting(protected_fields, self):
            ...
            cr = self.env.cr
            for model_name, name in self._inherits.items():
                parent_vals = inherited_vals[model_name]
                if not parent_vals:
                    continue

                # optimization of self.mapped(name)
                parent_ids = set()
                query = "SELECT %s FROM %s WHERE id IN %%s" % (name, self._table)
                for sub_ids in cr.split_for_in_conditions(self.ids):
                    cr.execute(query, [sub_ids])
    ...
    def create(self, vals):
        ...
        for model_name, name in self._inherits.items():
            parent_record = self.env[model_name].browse(store_vals.get(name))
            parent_vals = inherited_vals[model_name]
            if not parent_record:
                store_vals[name] = parent_record.create(parent_vals).id
            elif parent_vals:
