...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('odoo.addons.'):
            ...
            cls.test_tags = {'at_install', module}
...
def tag_test_selector(test, spec):
    ...
    """
    clean_conf_tags = {t.strip() for t in spec.split(',') if t.strip() != ''} or {'at_install'}
    ...
    tags = getattr(test, 'test_tags', set(['at_install', ])) # at_install for tests that does not inherit
    ...
    if not inter_to_test:
        ...
        return False
    if 'post_install' in to_include:
        if 'post_install' not in inter_to_test:
            _logger.debug("Test '{}' deselected because not a 'post_install' test".format(test))
            return False
        if not to_include.issubset(tags):
            _logger.debug("Test '{}' deselected because '{}' not in '{}'".format(test, to_include, tags))
            return False
    return True
