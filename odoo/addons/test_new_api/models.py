...
class ComputeReadWrite(models.Model):
    ...
    introduction = fields.Char(
        string='introduction', compute='_compute_introduction', readonly=False, store=True)
    author = fields.Many2one('res.users', default=lambda self: self.env.user)
    ...
    def _compute_introduction(self):
        for record in self:
            ...
            introduction += "The most exciting phrase to hear in science, the one that heralds "
            introduction += "new discoveries, is not \"Eureka!\" (I found it!) but \"That's funny...\"."
            introduction += "Isaac Asimov. Foundation."
    ...
    def _compute_parent(self):
        for compute in self:
            if compute.body:
                parent_id = len(compute.body)
                parent = self.search([('id', '=', parent_id)])
                if parent:
                    compute.parent = parent[0]
