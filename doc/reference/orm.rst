...
* stored computed fields are read-only by default. If the ``readonly``
  parameter is set to ``False``, the field is computed if a dependency
  changes and if a new value is not given.::

    taxes = fields.Integer(string='Taxes', default=20)
    amount = fields.Integer(
        string='Amount'compute='_compute_amount', readonly=False, store=True)
...
* check all ``onchange`` and replace them by computed fields if it's possible
