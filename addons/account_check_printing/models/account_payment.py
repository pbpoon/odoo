...
class AccountPayment(models.Model):
    ...
    def print_checks(self):
        ...
        if not self[0].journal_id.check_manual_sequencing:
            ...
            # Currently, framework does not support field type casting in search(), hence converted ORM search to query
            sql_query = """ SELECT check_number::int FROM account_payment
                WHERE journal_id = %s AND check_number::int <> 0
                ORDER BY check_number::int DESC LIMIT 1 """
            self._cr.execute(sql_query, (self[0].journal_id.id,))
            last_printed_check_number = self._cr.fetchone()
            next_check_number = last_printed_check_number and last_printed_check_number[0] + 1 or 1
            return {
