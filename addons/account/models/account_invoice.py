...
class AccountInvoice(models.Model):
    ...
    _inherit = ['mail.thread', 'mail.activity.mixin', 'portal.mixin', 'account.recurrent.mixin']
    ...
    @api.depends('date_invoice', 'state', 'is_recurrency_enabled', 'recurrency_interval', 'recurrency_type')
    def _compute_next_recurrency_date(self):
        for invoice in self.filtered(lambda inv: inv.state not in ('draft', 'cancel') and inv.is_recurrency_enabled and inv.recurrency_interval and inv.recurrency_type):
    ...
    def _create_draft_record(self):
        ...
        inv = self.copy(default=default)
        # Here the onchange will automatically write to the database
        inv._onchange_payment_term_date_invoice()
        inv.message_post(body=_('This bill has been generated by <a href=# data-oe-model=account.invoice data-oe-id=%d>%s</a>') % (self.id, self.number))
