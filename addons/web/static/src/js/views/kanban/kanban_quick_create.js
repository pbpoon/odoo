...
var RecordQuickCreate = Widget.extend({
    ...
    willStart: function () {
        ...
        if (this.formViewRef) {
            ...
            def2 = this.processFieldsViews({form: fieldsView}, fields);
        }
    ...
    _cancel: function () {
        var self = this;
        return this.controller.commitChanges().then(function () {
            self.controller.canBeDiscarded()
                .then(self.trigger_up.bind(self, 'cancel_quick_create'))
                .fail(self.controller.autofocus.bind(self.controller));
        });
    },
