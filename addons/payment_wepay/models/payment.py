...
class PaymentAcquirer(models.Model):
    ...
    def _get_wepay_getway(self, values):
        ...
        val = json.dumps({
            ...
            "type": "goods",  # goods, service, donation, event and personal.
            "currency": values.get('currency').name,
            "short_description": product_name,
            "callback_uri": urls.url_join(base_url, WepayController._notify_url),  # WePay need complete URL everytime (ex. www.example.com)
            "reference_id": values.get('reference'),
            "hosted_checkout": {
                "redirect_uri": urls.url_join(base_url, WepayController._return_url),
            },
            'fee': {
                'app_fee': values['fees'],
            }
...
class PaymentTransaction(models.Model):
    ...
    def _wepay_form_validate(self, data):
        ...
        if status in ['authorized', 'captured', 'refunded']:
            ...
            _logger.info('Validated Wepay payment for tx %s: set as done' % (self.reference))
            self.write({
                'state': new_state,
