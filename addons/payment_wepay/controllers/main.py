...
class WepayController(http.Controller):
    ...
    def wepay_dpn(self, **post):
        """ wepay DPN """
        _logger.info('Beginning wepay DPN form_feedback with post data %s', pprint.pformat(post))  # debug
        return_url = '/'  # wepay does not accept any custom variable like other payment acquirer so here we set home page while return to return url
        try:
    ...
    def wepay_ipn(self, **post):
        """ wepay IPN """
        _logger.info('Beginning wepay IPN form_feedback with post data %s', pprint.pformat(post))  # debug
        try:
            self._wepay_validate_data(**post)
        except ValidationError:
            _logger.exception('Unable to validate the WePay payment')
    ...
    @http.route('/wepay/account_done/<int:acq_id>', auth="user")
    def wepay_account_done(self, acq_id, **post):
        get_param = request.env['ir.config_parameter'].sudo().get_param
        wepay = request.env['payment.acquirer'].sudo().browse(acq_id)
        client_id = get_param('payment_wepay_%s_client_id' % wepay.environment)
        client_secret = get_param('payment_wepay_%s_client_secret' % wepay.environment)

        # Get access token of user
        val = {
            "client_id": client_id,
            "client_secret": client_secret,
            "redirect_uri": get_param('web.base.url') + "/wepay/account_done/"+str(acq_id),
            "code": post['code'],
        }
        response = requests.post(wepay.wepay_get_form_action_url()['token'], val)
        response.raise_for_status()
        vals = json.loads(response.text)
        wepay.write({
        ...
        user_details_response = requests.post(wepay.wepay_get_form_action_url()['user'], headers=wepay.get_wepay_header())
        ...
        response = requests.post(wepay.wepay_get_form_action_url()['create_account'], data=user_create_account, headers=wepay.get_wepay_header())
        response.raise_for_status()
        vals = json.loads(response.text)
        wepay.write({'wepay_account_id': vals.get('account_id')})
        return werkzeug.utils.redirect("/web#id="+str(wepay.id)+"&model=payment.acquirer&view_type=form")
