...
class ProjectTaskAssignSOLine(models.TransientModel):
    ...
    sale_line_id = fields.Many2one('sale.order.line', 'Sales Order Line', domain=[('is_service', '=', True)], help="Sale order line to link to selected tasks")
    partner_id = fields.Many2one('res.partner', related='sale_line_id.order_partner_id', readonly=True)
    task_ids = fields.Many2many('project.task', 'project_task_assign_so_line', 'task_id', 'wizard_id', string='Tasks')
