...
class MailTracking(models.Model):
    ...
    def create_tracking_values(self, initial_value, new_value, col_name, col_info):
        ...
        if col_info['type'] in ['integer', 'float', 'char', 'text', 'datetime', 'monetary']:
            ...
            values.update({
                'old_value_datetime': initial_value and datetime.strftime(datetime.combine(initial_value, datetime.min.time()), tools.DEFAULT_SERVER_DATETIME_FORMAT) or False,
                'new_value_datetime': new_value and datetime.strftime(datetime.combine(new_value, datetime.min.time()), tools.DEFAULT_SERVER_DATETIME_FORMAT) or False,
            })
