...
class HrExpense(models.Model):
    ...
    unit_amount = fields.Float(string='Unit Price', readonly=True, required=True, states={'draft': [('readonly', False)], 'reported': [('readonly', False)], 'refused': [('readonly', False)]}, digits=dp.get_precision('Product Price'))
    ...
    def upload_document(self):
        ...
        ctx.update({'default_res_model': 'hr.expense', 'default_res_id': self.id})
        return {
            'name': _('Upload a document'),
            'view_type': 'form',
            'view_mode': 'form',
        ...
        if help_message:
            ...
            if alias_record and alias_record.alias_domain and alias_record.alias_name:
                ...
                return '<p class="oe_view_nocontent_create">%s<br/>%s</p>%s' % (
                    ...
                    help_message)
        return '<p class="oe_view_nocontent_create">%s</p>%s' % (
            _('Click to add a new expense.'),
            help_message)
