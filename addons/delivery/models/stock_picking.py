...
class StockPicking(models.Model):
    ...
    weight_uom_id = fields.Many2one('product.uom', string='Unit of Measure', compute='_compute_weight_uom', help="Unit of measurement for Weight")
    ...
    def _compute_weight_uom(self):
        if self.env['ir.config_parameter'].sudo().get_param('stock.product_weight_in_lbs', '0') == '1':
            weight_uom = self.env.ref('product.product_uom_lb')
        else:
            weight_uom = self.env.ref('product.product_uom_kgm')
        for p in self:
