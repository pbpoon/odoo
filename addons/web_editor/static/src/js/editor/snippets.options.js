...
var SnippetOption = Widget.extend({
    ...
    _reset: function (force) {
        var self = this;
        var $actives = this.$el.find('.active').addBack('.active, [data-no-preview]');
     ...
     */
    _select: function (previewMode, $li, force) {
        var self = this;

        // Options can say they respond to strong choice
        if (!force && previewMode && ($li.data('noPreview') || $li.parent().data('noPreview'))) {
            return;
        }
