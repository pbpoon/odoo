...
class SaleOrder(models.Model):
    ...
    expected_date = fields.Datetime(compute='_compute_expected_date', string='Expected Date', store=True,
                                      help="Delivery date you can promise to the customer, computed "
                                      "from product lead times and from the shipping policy of the order.")
    commitment_date = fields.Datetime('Commitment Date', readonly=True, states={'draft': [('readonly', False)],
                                     'sent': [('readonly', False)]}, copy=False,
                                     help="This is the delivery date promised to the customer. If set, the delivery "
    ...
    def _compute_expected_date(self):
        ...
        for order in self:
            ...
            if dates_list:
                expect_date = min(dates_list) if order.picking_policy == 'direct' else max(dates_list)
                order.expected_date = fields.Datetime.to_string(expect_date)
